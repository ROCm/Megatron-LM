...
| **D5** | **CRIT — new** | **`fla`'s `chunk_kda` silently ignores the released call's arguments.** On fla `main` the op takes neither `A_log` nor `dt_bias`, names the state-layout flag `state_v_first` rather than `transpose_state_layout`, and ends its signature in `**kwargs` — so the released call raises nothing and runs with the learned decay ignored. The plan says "pin an fla revision whose `chunk_kda` signature matches the released HF call" and treats that as a formality; it is instead an unresolved blocking task, and the check must compare **parameter names** rather than "did the call raise". The published wheel (0.5.2) additionally ships no `fla/ops` at all. See `../notes/2026-08-27-fla-signature-check.md`. |
