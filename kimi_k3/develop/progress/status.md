...
| [x] | T6.4 EP path + `router_replay` determinism | TBD-g26 | 2026-08-28 | **G26 GREEN for the EP path**: 8 ranks, **112 local experts each** (896/8 exact), params per rank identical at **16,306,993,312**, **0 of 896 starved**, load max/mean 1.77. Determinism is **not** bitwise at bf16+EP and the cause is located — finding **A18**: the first MoE router is identical on every rank, so the source is inside the MoE accumulation, not our attention. **`router_replay` measured 2026-08-31**: it pins routing and cuts divergence **11.3x** on max-abs (0.705 → 0.0625), but the model is **still not bitwise** — residual rel-L2 5.0e-03 is the accumulation noise underneath. Not a determinism switch (`results/router_replay.md`) |
...
| [x] | T8.4 Checkpoint-anchored parity | `93b2c57ad` | 2026-08-27 | **G32 GREEN for one KDA layer on real weights**: 847 MiB fetched by range request, our module and the **release's own** module both load it with 0 missing / 0 unexpected, forward agrees to **rel-L2 7.4e-3, cosine 0.999973** (inside the measured bf16 bound). **Four-layer slice done 2026-08-31** (`results/anchored_parity_4l.md`): gated MLA **5.82e-03 / cosine 0.999983**, KDA layers 1-2 **7.8/7.4e-03**, AttnRes both sites **bit-identical**, routing **100 % identical top-16 sets** on the real `e_score_correction_bias`. Only the routed expert FFN forward is uncovered (59 GB/layer dequantised). Weights deleted after validation |
...
|     | Task | commit | date | note |
| --- | --- | --- | --- | --- |
| [x] | T11.1 EP=8 proxy + trace | `ef9bae4c1` | 2026-08-27 | **G42 GREEN.** 4 L official, **EP=8**, seq 512: steady **2653.5 ms** (spread 2643–2659) — **cold; warm the same configuration is 1912.6 ms, G50** — peak **193.6 GiB**/rank, 321 k launches. The earlier 2-layer run is kept as a depth cross-check — the ranking is unchanged |
