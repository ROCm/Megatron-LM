...
# export HIPBLASLT_LOG_FILE=qwen${MODEL_SIZE}B_gemm_shapes_${NODE_RANK}_gbs${BS}_mbs${MBS}.log 

# unset HIPBLASLT_LOG_MASK
# unset HIPBLASLT_LOG_FILE
# # unset HIPBLASLT_TUNING_OVERRIDE_FILE
# export HIPBLASLT_TUNING_OVERRIDE_FILE=hipblaslt_tuning_script_0_llama13b_gbs${BS}_result.log
...
DATA_PATH=${DATA_PATH:-"$DATA_DIR/qwen-datasets/wudao_qwenbpe_text_document"}
...
if [ $MODEL_SIZE = 0.5 ]; then
    ...
    echo "Model size not supported. Supported sizes are 0.5, 1.5, 7, 8, 32, 72"
    exit 1
fi
