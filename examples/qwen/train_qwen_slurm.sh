...
#SBATCH --output=logs/qwen/qwen32b.%j.out
...
#SBATCH --time=00:30:00                      # Adjust as necessary
...
export CONTAINER_NAME="training_env1"
...
export DATA_DIR=${DATA_DIR:-"${HOME}/dev/data"}
...
echo "IB_MOUNT_OPTIONS: $IB_MOUNT_OPTIONS"
...
srun bash -c '
    ${container_command} rm -f $(${container_command} ps -aq)
    ...
    ${container_command} run -d --network host --device /dev/dri --device /dev/kfd --device /dev/infiniband \
      --group-add video --cap-add SYS_PTRACE --security-opt seccomp=unconfined --privileged \
       ${IB_MOUNT_OPTIONS} \
      -v $HOST_MOUNT:$CONTAINER_MOUNT --shm-size 128G --name $CONTAINER_NAME $DOCKER_IMAGE tail -f /dev/null
'
...
echo "NUM_RECOMPUTE_LAYER: $NUM_RECOMPUTE_LAYER" 
echo "TP: $TP"
echo "PP: $PP"
...
srun bash -c '
  ${container_command} exec \
    ...
    -e NUM_RECOMPUTE_LAYER='"$NUM_RECOMPUTE_LAYER"' \
    -e FSDP='"$FSDP"' \
    ...
    bash -c "
      ...
      SEQ_LENGTH=\$SEQ_LENGTH bash examples/qwen/train_qwen.sh
    "
' 
