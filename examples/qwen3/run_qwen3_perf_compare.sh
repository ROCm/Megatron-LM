#!/bin/bash
###############################################################################
# Run Qwen3-235B-A22B proxy training across three precision configs (bf16,
...
#   - examples/qwen3/proxy_mi355x_qwen3_235B_A22B.sh    (proxy env file)
#   - examples/qwen3/train_qwen3.sh                     (Megatron launcher)
#   - TraceLens CLI tools on PATH (auto-installed from GitHub if missing):
#       TraceLens_generate_perf_report_pytorch
#       TraceLens_compare_perf_reports_pytorch
#   - Baseline CSV archive at the repo root:
#       qwen_baselines.zip
#     (auto-extracted into ${BASELINE_ROOT}/ on first run; contains
#      proxy_qwen3_bf16_baseline/, proxy_qwen3_fp8_baseline/,
#      proxy_qwen3_mxfp8_baseline/)
#
# Usage:
#   cd /path/to/Megatron-LM
#   bash examples/qwen3/run_qwen3_perf_compare.sh                 # all 3
#   bash examples/qwen3/run_qwen3_perf_compare.sh bf16            # only bf16
#   bash examples/qwen3/run_qwen3_perf_compare.sh bf16 fp8        # subset
#   bash examples/qwen3/run_qwen3_perf_compare.sh all             # explicit
#   PRECISIONS="fp8,mxfp8" bash examples/qwen3/run_qwen3_perf_compare.sh
#
# Precisions (positional args or PRECISIONS env var, space- or comma-
# separated). Valid tokens: bf16 | fp8 | mxfp8 | all (default: all).
#
# Optional overrides:
#   RUNS_ROOT=...           (default: tracelens_runs/qwen3_<TS>)
...
#   BASELINE_ZIP=...        (default: qwen_baselines.zip)
#   BASELINE_ROOT=...       (default: qwen_baselines; extraction target dir)
#   BASELINE_BF16_DIR=...   (default: ${BASELINE_ROOT}/proxy_qwen3_bf16_baseline)
#   BASELINE_FP8_DIR=...    (default: ${BASELINE_ROOT}/proxy_qwen3_fp8_baseline)
#   BASELINE_MXFP8_DIR=...  (default: ${BASELINE_ROOT}/proxy_qwen3_mxfp8_baseline)
...
source examples/qwen3/proxy_mi355x_qwen3_235B_A22B.sh

TS=$(date +%Y%m%d_%H%M%S)
RUNS_ROOT=${RUNS_ROOT:-"tracelens_runs/qwen3_${TS}"}
mkdir -p "${RUNS_ROOT}"

BASELINE_ZIP=${BASELINE_ZIP:-"qwen_baselines.zip"}
BASELINE_ROOT=${BASELINE_ROOT:-"qwen_baselines"}
BASELINE_BF16_DIR=${BASELINE_BF16_DIR:-"${BASELINE_ROOT}/proxy_qwen3_bf16_baseline"}
BASELINE_FP8_DIR=${BASELINE_FP8_DIR:-"${BASELINE_ROOT}/proxy_qwen3_fp8_baseline"}
BASELINE_MXFP8_DIR=${BASELINE_MXFP8_DIR:-"${BASELINE_ROOT}/proxy_qwen3_mxfp8_baseline"}
...
# repo root and contains the proxy_qwen3_*_baseline/ directories at top level.
extract_baselines() {
...
#   rest = extra `KEY=VAL` pairs to prepend before `bash train_qwen3.sh`
run_train() {
    local tag="${1:-}"
    if [ -z "${tag}" ]; then
    ...
    # train_qwen3.sh respects OUTPUT_BASEPATH if already set.
    # Use `env` to inject per-run vars without polluting our shell.
    env OUTPUT_BASEPATH="${out_base}" "$@" \
        bash examples/qwen3/train_qwen3.sh 2>&1 | tee "${train_log}"
}
