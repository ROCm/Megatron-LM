...
def should_free_input(name, is_moe, config, num_local_experts):
    ...
    if not free_mlp:
        # AlltoAll dispatcher with local_num_experts=1, HybridEP, MORI, and NCCL EP all use
        ...
        free_mlp = free_mlp and not (enable_hybridep or enable_ncclep or enable_mori)

    free_input_nodes = {
        "mlp": free_mlp,
        "moe_combine": True,
        # For non-DeepEP/HybridEP/MORI/NCCL-EP dispatcher mode, the input is the un-dispatched
        # tokens and probs before dispatch A2A and it's not needed anymore after the
        # forward pass. For DeepEP, HybridEP, MORI, and NCCL EP dispatcher mode, they are all
        ...
        "moe_dispatch": not (enable_deepep or enable_hybridep or enable_ncclep or enable_mori)
